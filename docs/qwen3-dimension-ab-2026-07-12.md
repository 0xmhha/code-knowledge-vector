...
- **N=4 측정 가능 쿼리**(나머지 6은 타겟이 서브셋 밖) — 방향성. 대형 코퍼스·다중 fixture 재확인 권장.
- ADR-008은 *차원* 축을 4b@1024로 락했다. 본 결과는 *모델* 축에서 **0.6b로의 전환을 시사** —
  대형 재확인에서 동등/우위가 유지되면 권장 모델을 4b→0.6b로 조정하고 ADR-008 후속 갱신.
- 남은 것: `knownDims` 표준화, 대형 코퍼스 모델-크기 재확인.
