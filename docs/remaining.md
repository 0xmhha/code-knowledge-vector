...
- [~] **flow Phase C→F** — 진행 중. **C1 완료(2026-07-12)**: flow_step의 file:line을 같은 인덱스의 코드 청크에 정렬 → `FlowStepMeta.AlignedChunkID` 스탬프. `internal/flowcorpus/align.go`(`CodeIndex`/`AlignSteps`, tightest-containing-span 매칭). 빌드가 source 루프에서 코드 청크 span 누적(symbol/function_split만) 후 flow load 시 정렬, 해소율 로그 + <80% warn. reindex의 flow 재임베딩(P3b-flow)도 store 조회로 재정렬(`alignFlowStepsFromStore`) — 정렬 무손실. 테스트 `TestAlignSteps_*`·`TestRun_AlignsFlowStepsToCode`·`TestReindex_RealignsFlowStepsToCode`. **C2 잔여**: 서빙 시 drift step stale 플래그(`EnforceCitationsAt`에 flow_step 포함) — 인프라(`StaleCitations`)는 이미 존재. **E/F**: 빌드 오케스트레이션(일부 `build-knowledge.sh`)·평가. CKS 표면 노출(Phase D)은 CKS 소관.
