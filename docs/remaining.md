...
- [~] **flow Phase C→F** — **Phase C 완료(2026-07-12)**. **C1**: flow_step의 file:line을 같은 인덱스의 코드 청크에 정렬 → `FlowStepMeta.AlignedChunkID` 스탬프. `internal/flowcorpus/align.go`(`CodeIndex`/`AlignSteps`, tightest-containing-span). 빌드가 source 루프에서 코드 span 누적(symbol/function_split만) 후 정렬·해소율 로그·<80% warn; reindex도 store 조회로 재정렬(`alignFlowStepsFromStore`). **C2**: 서빙 시 drift 감지 — `EnforceCitationsAt`가 flow_step의 인용 라인이 현재 파일 길이를 넘으면 `StaleCitation=true`(drop 아님, warning 표면화). flow는 CommitHash가 비어 commit-축 stale이 안 걸리므로 라인-바운드가 유일 신호(commit 독립). 테스트 `TestAlignSteps_*`·`TestRun_AlignsFlowStepsToCode`·`TestReindex_RealignsFlowStepsToCode`·`TestEnforceCitationsAt_FlowStepLineDrift`. **Phase D 완료(정정)**: flow-aware 도구 4종이 CKV에 구현·MCP 노출됨 — `internal/query/flow.go`(`GetFlow`/`ExpandFlow`/`FindBranches`/`GetInvariantEnforcement`) + `pkg/mcp/server.go`(`cks.context.get_flow`/`expand_flow`/`find_branches`/`get_invariant_enforcement`). (표면 *소비*만 CKS 소관). **E 대부분 완료(2026-07-12)**: 빌드 오케스트레이션 스크립트(`scripts/build-knowledge.sh`) + 머신-로컬 경로를 `build-profiles.env`(git-ignored)로 외부화, 커밋 템플릿 `build-profiles.env.example`, Makefile·`pr-retrieval-eval.sh`의 하드코딩 경로 제거(잔존 0). **잔여 F**: flow-queries 정답셋 평가(`get_flow`/`find_branches` precision/recall) — 대형 corpus(255레코드) 의존이라 정본 데이터셋 확보 후.
- [x] **ADR 승격(F)** — 완료. canonical_id join(**ADR-007**), 임베딩 차원(**ADR-008**), retrieval prefix/granularity 전략(**ADR-009**), flow 시그니처(**ADR-010**: 레코드→청크 종류·ID 스킴·임베드 텍스트·content_hash 통째 재인덱싱, 2026-07-12).
