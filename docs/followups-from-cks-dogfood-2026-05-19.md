...
| CKV-7 | ✅ ckv-side done | `cks.context.semantic_search` response schema versioning | cks parses the MCP tool result content opportunistically | **2026-05-21: 완료.** 모든 MCP tool response 의 top-level 에 `schema_version` 문자열 inject (현재 `"1"`). `pkg/mcp/server.go::jsonResult` 가 마샬링 시 자동 inject — 새 tool 추가 시 누락 불가능 (구조적 보장). 정책: minor (1 → 1.1) = additive, major (1 → 2) = breaking. cks-side action: `parseSemanticSearchResult` / `parseHealthResult` 등에서 `schema_version` 의 major part 를 확인 → mismatch 시 warning 로그 + 마지막으로 알려진 fields fallback. 4 tools (semantic_search / health / warmup / get_freshness) 모두 `TestAllToolResponsesCarrySchemaVersion` table-driven test 로 contract 검증. |
