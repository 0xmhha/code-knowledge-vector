...
**4곳 수정:**
1. `internal/ckvclient/interface.go`: `Client`에 `FindInvariants(ctx, file, category, tierMin)` /
   `GetConventions(ctx, packagePrefix)` 추가 + 신규 contract 타입
...
2. `internal/ckvclient/real.go`: `r.eng.FindInvariants(...)` / `GetConventions(...)` 호출 +
   ckv 타입 → contract 타입 번역 (SemanticSearch 번역 미러).
3. `internal/ckvclient/dummy.go` + `fake.go`: degraded(빈 결과) + 테스트 fake 구현.
...
1. 이 문서 + `coding-agent/docs/knowledge-system-analysis-2026-06-17.md` 읽기.
2. **Stream A의 CKS-side(§3)부터** — `code-knowledge-system` repo. contract 타입 신설 →
   real/dummy/fake → MCP 등록 → cks `go build && go test ./...`.
3. e2e 확인 후 Stream B(flow corpus) 착수.
4. doctor(D)는 여유 시 병행.

> 주의: ckv 빌드/테스트는 gvm 환경이라 login 셸 필요 — `zsh -lic 'make build && go test ./...'`.
> `internal/embed/coreml`는 libtokenizers 부재로 로컬 test 1건 실패(환경 baseline, CI는 제외).
